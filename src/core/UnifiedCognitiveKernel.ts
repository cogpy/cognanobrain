...
import { GgmlTensorKernel, AtomSpaceTensorConfig } from './GgmlTensorKernel';
...
export class UnifiedCognitiveKernel {
  ...
  processAtoms(atoms: AtomSpaceAtom[], links: AtomSpaceLink[]): ProcessingResult {
    ...
    try {
      ...
      if (nodeTensors.length > 0) {
        const attentionStats = this.attentionEngine.getCurrentStats(nodeTensors as any);
