...
/**
 * Cognitive kernel hook state
 */
interface CognitiveKernelHookState {
  kernel: UnifiedCognitiveKernel | null;
  isInitialized: boolean;
  isRunning: boolean;
  stats: CognitiveKernelStats | null;
  error: string | null;
...
interface ProcessingOptions {
  enableAttentionAllocation?: boolean;
  enableRecursiveReasoning?: boolean;
...
export function useCognitiveKernel() {
   ...
   */
  const updateKernelConfig = useCallback((config: any) => {
    if (!state.kernel) return;
   ...
   */
  const integrateCognitiveNodes = useCallback((cognitiveNodes: any[]) => {
